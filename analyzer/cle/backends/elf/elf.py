...
class ELF(MetaELF):
    ...
    class low_high_pc:
        ...
        '''
        def load_from_DIE(self, die:DIE, dwarf) -> (int, int):
            rtype = self.get_range_type(die)

            if rtype == self.range_type.CONTINUOUS:
                return self.load_from_DIE_continuous(die)
            
            elif rtype == self.range_type.NON_CONTINUOUS:
                return self.load_from_DIE_ranges(die, dwarf)
        ...
        """
        def load_from_DIE_continuous(self, die:DIE) -> (int, int):
        ...
        '''
        def load_from_DIE_ranges(self, die:DIE, dwarf:DWARFInfo) -> (int, int):
    ...
    def _load_dies(self, dwarf: DWARFInfo):
        ...
        for cu in dwarf.iter_CUs():
            ...
            die_low_pc, die_high_pc = self.low_high_pc().load_from_DIE(top_die, dwarf) #------------ added by jingyu ----------------
            ...
            for die_child in cu.iter_DIE_children(top_die):
                if die_child.tag == "DW_TAG_variable":
                    ...
                    # load subprogram
                    sub_prog = self._load_die_lex_block(die_child, expr_parser, type_list, cu, cu_.file_path, None, dwarf)
    ...
    def _load_die_lex_block(self, die: DIE, expr_parser, type_list, cu, file_path, subprogram, dwarf:DWARFInfo) -> LexicalBlock:
        ...
        low_pc, high_pc = self.low_high_pc().load_from_DIE(die, dwarf) #------------ added by jingyu ----------------
        ...
        for sub_die in cu.iter_DIE_children(die):
            if sub_die.tag in ["DW_TAG_variable", "DW_TAG_formal_parameter"]:
                ...
                sub_block = self._load_die_lex_block(sub_die, expr_parser, type_list, cu, file_path, subprogram, dwarf)
